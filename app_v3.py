...
def load_data_r_gap(file_path):
    """R_analysis.txt 에서 '틈새 거리'를 추출하는 로더"""
    text_data = get_text_from_github(file_path)
    if not text_data: return None, None
    ...
    for block in blocks:
        ...
        gap_match = re.search(r"▶ 틈새 거리:\s*([\d\.]+)\s*px", block)
...
def show_CG_page(font_name):
    ...
    if df is not None:
        ...
        with col1:
            ...
            event = st.plotly_chart(fig, on_select="rerun", selection_mode="points", use_container_width=True, key=f"chart_{font_name}")
...
def show_U_page():
    ...
    if df is not None:
        ...
        with col1:
            ...
            event = st.plotly_chart(fig, on_select="rerun", selection_mode="points", use_container_width=True, key="chart_U")
...
def render_2d_page(title, df, centroids, file_path, result_folder, suffix=""):
    ...
    with col1:
        ...
        event = st.plotly_chart(fig, on_select="rerun", selection_mode="points", use_container_width=True, key=f"chart_2d_{result_folder}_{suffix}")
...
if menu == "C 폰트 분석":
    ...
    if all(d is not None for d in [df_c, df_g, df_u]):
        for d in [df_c, df_g, df_u]:
            d['orig_fname'] = d['filename'].apply(get_orig_fname)
    ...
    if df_thick is not None and df_width is not None and df_gap is not None:
        ...
        df_gap.drop_duplicates(subset=['filename'], keep='first', inplace=True)
        ...
        merged_df = pd.merge(
            merged_df,
            df_gap[['filename', 'cluster', 'value']].rename(columns={'cluster': 'cluster_gap', 'value': 'gap_val'}),
            on='filename'
        )
        
        st.subheader("전체 교집합 분포 현황 (두께, 너비, 틈새)")
        dist_df = merged_df.groupby(['cluster_thick', 'cluster_width', 'cluster_gap']).size().reset_index(name='데이터 수')
        ...
        with sel_col3: sel_gap = st.selectbox("틈새 거리 그룹 선택", ['0', '1', '2'])
            
        common_df = merged_df[(merged_df['cluster_thick'] == sel_thick) & (merged_df['cluster_width'] == sel_width) & (merged_df['cluster_gap'] == sel_gap)]
        st.write(f"**두께 {sel_thick} & 너비 {sel_width} & 틈새 {sel_gap}** 조합의 폰트는 총 **{len(common_df)}**개 입니다.")
        
        if not common_df.empty:
            st.dataframe(common_df[['filename', 'thick_val', 'width_val', 'gap_val']], use_container_width=True)
            ...
            with st.expander(f"선택된 교집합 전면 이미지 모두 보기", expanded=False):
                ...
                for _, row in common_df.iterrows():
                    ...
                    if matched_file:
                        ...
                        if img:
                            with cols[col_idx % 5]:
                                st.image(img, caption=f"{prefix}\n({row['thick_val']}px, {row['width_val']}px, {row['gap_val']}px)", use_container_width=True)
                            col_idx += 1
                if col_idx == 0: st.write("전면 이미지를 찾을 수 없거나 불러올 수 없습니다.")
    else:
    ...
    st.markdown("모든 알파벳(C, G, U, R 두께, R 너비, R 틈새)의 특정 군집 조건을 **모두 만족하는 폰트**를 한 번에 필터링합니다.")
    ...
    if all(d is not None for d in [df_c, df_g, df_u, df_r_thick, df_r_width, df_r_gap]):
        
        for d in [df_c, df_g, df_u, df_r_thick, df_r_width, df_r_gap]:
            d['orig_fname'] = d['filename'].apply(get_orig_fname)
            d.drop_duplicates(subset=['orig_fname'], keep='first', inplace=True)
        ...
        rg_sub = df_r_gap[['orig_fname', 'cluster']].rename(columns={'cluster': 'R_Gap_group'})
        ...
        merged = pd.merge(merged, rg_sub, on='orig_fname', how='inner')
        ...
        with c6: sel_rg = st.selectbox("R 틈새 그룹", ['0', '1', '2'])
        ...
        final_df = merged[(merged['C_group'] == sel_c) & 
                          ...
                          (merged['R_Gap_group'] == sel_rg)]
        
        st.success(f"이 6가지 조건을 모두 만족하는 완벽한 교집합 폰트는 총 **{len(final_df)}**개 입니다.")
