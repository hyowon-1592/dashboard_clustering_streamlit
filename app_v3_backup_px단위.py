...
@st.cache_data
def load_data_location(file_path):
    """중심 좌표와 빨간 점의 'x 차이'를 추출하는 로더"""
    text_data = get_text_from_github(file_path)
    if not text_data: return None, None
...
nav_button("R 다리 시작점과 중심 좌표의 거리 클러스터링")
...
if menu == "C 폰트 분석":
    ...
    st.markdown("C, G, U 폰트 각각의 그룹 조합을 선택하여 **세 알파벳 모두 지정한 특성 그룹에 속하는 폰트**를 찾습니다.")
...
elif menu == "R 다리 시작점과 중심 좌표의 거리 클러스터링":
    ...
    if df_loc is not None and df_gap is not None:
        df_loc.drop_duplicates(subset=['filename'], keep='first', inplace=True)
        df_gap.drop_duplicates(subset=['filename'], keep='first', inplace=True)

        merged_df = pd.merge(
            df_loc[['filename', 'cluster', 'value']].rename(columns={'cluster': 'cluster_loc', 'value': 'loc_val'}),
            df_gap[['filename', 'cluster', 'value']].rename(columns={'cluster': 'cluster_gap', 'value': 'gap_val'}),
            on='filename'
        )
        
        st.subheader("전체 교집합 분포 현황 (중심과의 거리, 유격)")
        dist_df = merged_df.groupby(['cluster_loc', 'cluster_gap']).size().reset_index(name='데이터 수')
            ...
            with st.expander(f"선택된 교집합 전면 이미지 모두 보기", expanded=False):
                ...
                for _, row in common_df.iterrows():
                    ...
                    if matched_file:
                        ...
                        if img:
                            with cols[col_idx % 5]:
                                st.image(img, caption=f"{prefix}\n({row['loc_val']}px, {row['gap_val']}px)", use_container_width=True)
                            col_idx += 1
                if col_idx == 0: st.write("전면 이미지를 찾을 수 없거나 불러올 수 없습니다.")
    else:
    ...
    st.markdown("모든 알파벳(C, G, U, R location, R 틈새)의 특정 군집 조건을 **모두 만족하는 폰트**를 한 번에 필터링합니다.")
    ...
    if all(d is not None for d in [df_c, df_g, df_u, df_r_loc, df_r_gap]):
        ...
        for d in [df_c, df_g, df_u, df_r_loc, df_r_gap]:
            d['orig_fname'] = d['filename'].apply(get_orig_fname)
            d.drop_duplicates(subset=['orig_fname'], keep='first', inplace=True)
        ...
        rloc_sub = df_r_loc[['orig_fname', 'cluster']].rename(columns={'cluster': 'R_Location_group'})
        ...
        merged = pd.merge(merged, rloc_sub, on='orig_fname', how='inner')
        ...
        with c4: sel_rloc = st.selectbox("R location 그룹", ['0', '1', '2'])
        with c5: sel_rg = st.selectbox("R 틈새 그룹", ['0', '1', '2'])
        
        final_df = merged[(merged['C_group'] == sel_c) & 
                          (merged['G_group'] == sel_g) & 
                          (merged['U_group'] == sel_u) & 
                          (merged['R_Location_group'] == sel_rloc) & 
